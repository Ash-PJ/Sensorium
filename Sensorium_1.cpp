...
int main()
{
    ...
    hSerial = CreateFile(TEXT("COM3"), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
            ...
            if (SUCCEEDED(hr)) {
                ...
                for (int i = 0; i < numRows; i++) {
                    for (int j = 0; j < numCols; j++) {
                        ...
                        // Get center pixel value of the current group
                        int centerX = j * groupWidth + groupWidth / 2;
                        int centerY = i * groupHeight + groupHeight / 2;
                        int pixelValue = normalizedMat.at<uchar>(centerY, centerX);

                        // Send the pixel value to the Arduino
                        std::string data;
                        for (int i = 0; i < numRows; i++) {
                            for (int j = 0; j < numCols; j++) {
                                int centerX = j * groupWidth + groupWidth / 2;
                                ...
                                // Determine the thresholded value based on the average value
                                ...
                                data += std::to_string(thresholdedValue);
